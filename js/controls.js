...
function makeSlider(label, minVal = 0, maxVal = 50, value = 10, step = 1, parent = createDiv(), update = () => {}) {
  ...
  function makeInput(label, parent, callback, type = "not_modal") {
    let wrapper = createDiv(label);
    ...
    buttonWrapper.class("input-wrapper");
    wrapper.class("input-wrapper");
    let button = createButton("应用");
    let inputtext = createInput();
    button.class("apply");
    inputtext.class("inputfield");
    if(type === "modal") {
        button.attribute("data-toggle", "modal")
        button.attribute("data-target", "#exampleModal")
    }
    button.parent(buttonWrapper)
    inputtext.parent(inputWrapper)
    inputWrapper.parent(wrapper);
    buttonWrapper.parent(wrapper);
    button.mousePressed(callback);
  }